...
Spectrum WhittedIntegrator::li(Ray& ray, const Scene& scene, std::shared_ptr<Sampler> sampler) const {
    ...
    do {
        ...
        if (auto light = its.shape->light; light) { spectrum += beta * light->evaluateEmission(its, -ray.direction); }
        ...
        // If the surface is not specular, sample the light
            ...
            for (auto light : scene.infiniteLights) {
                ...
                Ray  shadowRay(its.position, lightSampleResult.direction, 1e-4f, FLT_MAX);
            ...
            if (light && pdfLight != .0f) {
                auto lightSampleResult = light->sample(its, sampler->next2D());
                Ray  shadowRay(its.position, lightSampleResult.direction, 1e-4f, lightSampleResult.distance);
                // Successfully connect the light source
                if (auto occlude = scene.rayIntersect(shadowRay); !occlude.has_value()) {
                    Spectrum f = bsdf->f(-ray.direction, shadowRay.direction);
